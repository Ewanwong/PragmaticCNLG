...
class PragmaticGPT2LMHeadModel(GenerationMixin):
    def __init__(self, model, alpha, epsilon, num_classes, prior_aggregation_method="sum"):
        self.config = AutoConfig.from_pretrained(model) 
        super().__init__()
              
        self._device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        #self._device = torch.device("cpu")
        self.model = AutoModelForCausalLM.from_pretrained(model).to(self._device)
    ...
    def compute_prior_distributions(self, 
        ...
        left_padded_loss_by_sequence = torch.zeros(loss_by_sequence.shape[0], loss_by_sequence.shape[1]).to(self._device)
    ...
    def compute_perplexity(self, input_texts, target_prompts, distractor_prompts):
        ...
        regular_logits = logits[:1, ...] # 1 * len * vocab
        
        real_labels = labels[:1, ...] # 1 * len
        
        label_mask = (labels!=-100) # 1 * len
        real_label_mask = (real_labels!=-100)
        real_input_length = real_label_mask.sum(dim=1).item()
        
        real_output_logits = logits[label_mask, :].view(self.num_classes+1, -1, logits.shape[-1]) # bsz * real_len * vocab
        
        real_prior_distribution = prior_distributions[:, -real_input_length:, :] # bsz * real_len * num_classes
        
        prob = F.softmax(real_output_logits, dim=-1)
        regular_prob = prob[:1,:,:] # 1 * real_len * vocab
        other_prob = prob[1:,:,:] # num_classes, len, vocab

        other_prob_by_example = other_prob.view(self.num_classes, 1, other_prob.shape[1] , other_prob.shape[-1]).permute(1, 2, 0, 3) # real_bsz * len * num_classes * vocab
        
        pragmatic_listener_probability_distribution = torch.mul(other_prob_by_example, real_prior_distribution.unsqueeze(-1).expand(-1, -1, -1, other_prob.shape[-1])) # real_bsz * len * num_classes * vocab
        pragmatic_listener_probability_distribution = pragmatic_listener_probability_distribution / pragmatic_listener_probability_distribution.sum(dim=2).unsqueeze(dim=2) # real_bsz * len * num_classes * vocab -> prior for next step depending on what token is selected

        pragmatic_speaker_probability_distribution = torch.mul(pragmatic_listener_probability_distribution[:,:,0,:] ** self.alpha, regular_prob)
        pragmatic_speaker_probability_distribution = pragmatic_speaker_probability_distribution / pragmatic_speaker_probability_distribution.sum(dim=-1).unsqueeze(dim=-1) # real_bsz * len * vocab
        
        pragmatic_logits = torch.log(pragmatic_speaker_probability_distribution)
        
        real_labels = real_labels[real_label_mask].unsqueeze(0)
        
        real_logits = regular_logits[real_label_mask, :].unsqueeze(0)
        

        shift_regular_logits = real_logits[..., :-1, :].contiguous()
