...
def run_self_diagnosis_experiment(model: PragmaticGPT2LMHeadModel, examples: List[ModelOutput], attribute_name: str, target_prompt: List[str],
    ...
    return {'test_pearsonr': test_corr, 'test_accuracy': test_accuracy, 'dev_pearsonr': dev_corr, 'dev_accuracy': best_dev_accuracy}, all_test_examples, predicted_scores
...
if __name__ == '__main__':
    ...
    parser.add_argument("--examples_filename", type=str, default="realtoxicityprompts-data/generations/prompted/prompted_gens_gpt2.jsonl",
    ...
    parser.add_argument("--error_filename", type=str, required=True,
                        help="Path to a file to which the errors are written")
    parser.add_argument("--models", type=str, nargs='+', default=['gpt2', 'gpt2-xl'],          # ['gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl']
    ...
    parser.add_argument("--batch_sizes", type=int, nargs='+', default=[16, 2],
    ...
    parser.add_argument("--n_demonstrations", type=int, default=2,
                        help="number of demonstrations to show the model")
    parser.add_argument("--failure_demonstrations", action='store_true',
                        help="whether to use examples where model fails as demonstrations or not")
    ...
    if args.use_keywords:
        descriptions = DESCRIPTIONS_KEYWORDS
    else:
    ...
    for model_idx, model_name in enumerate(args.models):
        ...
        model = PragmaticGPT2LMHeadModel(model_name, 0, 0, len(TARGET_PREFIXES)+len(DISTRACTOR_PREFIXES), args.prior_aggregation_method).to(device)


        batch_size = args.batch_sizes[model_idx] if isinstance(args.batch_sizes, list) else args.batch_sizes

        for attribute in args.attributes:
            correct[model_name][attribute] = []
            incorrect[model_name][attribute] = []
            #if args.failure_demonstrations:
                
            target_prompt = fill_in_prefix(TARGET_PREFIXES, descriptions, attribute, args.n_demonstrations, True)
            distractor_prompts = fill_in_prefix(DISTRACTOR_PREFIXES, descriptions, attribute, args.n_demonstrations, False)
            print(target_prompt)
            print(distractor_prompts)
            result, all_test_examples, predicted_scores = run_self_diagnosis_experiment(
            ...
            for example in all_test_examples:
                if example.scores[attribute] > 0.5:
                    if predicted_scores[example] > 0.5:
                        correct[model_name][attribute].append({'text':example.text.replace('\n', ''), 'pred': predicted_scores[example], 'actual': example.scores[attribute]})
                    else:
                        incorrect[model_name][attribute].append({'text':example.text.replace('\n', ''), 'pred': predicted_scores[example], 'actual': example.scores[attribute]})
                else:
                    if predicted_scores[example] > 0.5:
                        incorrect[model_name][attribute].append({'text':example.text.replace('\n', ''), 'pred': predicted_scores[example], 'actual': example.scores[attribute]})
                    else:
    ...
    with open(f"{args.error_filename}_correct.json", 'w', encoding='utf8') as fh:
        json.dump(correct, fh, indent=4)
        
    with open(f"{args.error_filename}_wrong.json", 'w', encoding='utf8') as fh:
        json.dump(incorrect, fh, indent=4)